...
static uint32_t s_uiWitDataCnt = 0, s_uiProtoclo = WIT_PROTOCOL_905x_CAN, s_uiReadRegIndex = 0;
    ...
    switch(s_uiProtoclo)
    {
		...
		case WIT_PROTOCOL_905x_CAN:
			...
			if (s_uiWitDataCnt >= 8)
			{
				...
				usData[1] = ((uint16_t)s_ucWitDataBuff[5] << 8) | (uint16_t)s_ucWitDataBuff[4];
				usData[2] = ((uint16_t)s_ucWitDataBuff[7] << 8) | (uint16_t)s_ucWitDataBuff[6];
				CopeWitData(s_ucWitDataBuff[1], usData, 3);
				WitCanDataIn(s_ucWitDataBuff, s_uiWitDataCnt);
				s_uiWitDataCnt = 0;
			}
			p_WitRegUpdateCbFunc(0, 0);
			break; 
