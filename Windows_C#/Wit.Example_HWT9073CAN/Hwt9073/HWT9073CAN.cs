...
namespace Wit.SDK.Modular.WitSensorApi.Modular.HWT9073
{

    /// <summary>
    /// HWT9073CAN连接类
    /// </summary>
    public class HWT9073CAN : IAttitudeSensorApi
    {
        /// <summary>
        /// 设备模型
        /// </summary>
        ...
        /// 记录数据委托
        /// </summary>
        public delegate void OnRecordHandler(HWT9073CAN HWT9073CAN);
        ...
        /// 设置CAN端波特率
        /// </summary>
        public void SetCanRate(string baudRate) 
        {
            DeviceModel.SetDeviceData(InnerKeys.CanBaud, baudRate.Trim().Replace("K", "000").Replace("M", "000000"));
        }
        ...
        public void SetCANId(byte CANId)
        {
            if (DeviceModel.IsOpen)
            {
                try
                {
                    DeviceModel.DeviceId = CANId.ToString();
                    SendProtocolData(new byte[] { 0xff, 0xaa, 0x1A, CANId, 0x00, });
                }
                catch (Exception)
                {
            ...
            // 更改上位机地址
            DeviceModel.SetAddr(CANId.ToString());
        }
